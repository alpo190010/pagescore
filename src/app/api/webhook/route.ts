...
async function generateFullReport(url: string): Promise<string> {
  ...
  try {
    const res = await fetch(url, {
      headers: { "User-Agent": "Mozilla/5.0 (compatible; PageScore/1.0)" },
...
async function sendReportEmail(
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "PageScore <report@pagescore.app>",
      to: email,
      subject: `Your PageScore Report: ${url}`,
      html: `
        <div style="font-family: system-ui, sans-serif; max-width: 680px; margin: 0 auto; padding: 40px 20px;">
          <h1 style="font-size: 24px; margin-bottom: 8px;">Your PageScore Report</h1>
          <p style="color: #666; margin-bottom: 32px;">Analysis for: <a href="${url}">${url}</a></p>
          <div style="white-space: pre-wrap; line-height: 1.6;">${report.replace(/\n/g, "<br>").replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>")}</div>
          <hr style="margin: 40px 0; border: none; border-top: 1px solid #eee;">
          <p style="color: #999; font-size: 12px;">Generated by PageScore — AI Landing Page Analyzer</p>
        </div>
      `,
    }),
  });

...
async function generateFullReport(url: string): Promise<string> {
  ...
  try {
    const res = await fetch(url, {
      headers: { "User-Agent": "Mozilla/5.0 (compatible; PageLeaks/1.0)" },
...
async function sendReportEmail(
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "PageLeaks <report@pageleaks.com>",
      to: email,
      subject: `Your PageLeaks Report: ${url}`,
      html: `
        <div style="font-family: system-ui, sans-serif; max-width: 680px; margin: 0 auto; padding: 40px 20px;">
          <h1 style="font-size: 24px; margin-bottom: 8px;">Your PageLeaks Report</h1>
          ...
          <p style="color: #999; font-size: 12px;">Generated by PageLeaks — AI Landing Page Analyzer</p>
        </div>
      `,
    }),
  });
